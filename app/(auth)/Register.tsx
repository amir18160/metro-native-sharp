...
export default function RegisterScreen() {
    ...
    const validateAndRegister = () => {
        ...
        if (password.length < 8) {
            return modal.error({ title: 'خطا', message: 'رمز عبور باید حداقل ۸ کاراکتر باشد.' });
        }
        if (!/[a-z]/.test(password)) {
            return modal.error({
                title: 'خطا',
                message: 'رمز عبور باید شامل حرف کوچک انگلیسی باشد.',
            });
        }
        if (!/[A-Z]/.test(password)) {
            return modal.error({
                title: 'خطا',
                message: 'رمز عبور باید شامل حرف بزرگ انگلیسی باشد.',
            });
        }
